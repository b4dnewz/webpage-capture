...
import { devices, WebpageCapture } from "../lib/index";
...
describe("webpage-capture", () => {
  ...
  describe("constructor", () => {
    ...
    it("throw on invalid viewport", () => {
      expect(() => {
        const instance = new WebpageCapture({
          viewport: "invalid",
        });
      }).toThrow();
    });
  ...
  afterEach(() => {
    _.flattenDeep(outputCollector).forEach((f) => {
      fs.unlinkSync(f);
    });
    outputCollector = [];
  });
  ...
  describe(".file", () => {
    it("return the output file path", async () => {
      const outFile = path.resolve(outputDir, "about");
    ...
    it("inherit output type from filename", async () => {
      const outFile = path.resolve(outputDir, "about.jpeg");
      const res = await capturer.file("about:blank", outFile);
      outputCollector.push(res.output);
      expect(res.input).toEqual("about:blank");
