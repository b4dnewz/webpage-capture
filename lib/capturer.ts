...
type CaptureType = "buffer" | "base64" | "html" | "pdf" | "jpeg" | "png";

/**
 * A set of options used to customize the request
 */
interface RequestOptions {
  ...
  /**
   * Set the event that page will wait
   */
...
/**
 * Options used to customize the captured output
 * and the capture execution such as viewports, headers and more
 */
interface CaptureOptions extends RequestOptions {
...
interface InstanceOptions extends Pick<Options, Exclude<keyof Options, "viewport">> {
...
interface RunOptions extends Pick<CaptureOptions, Exclude<keyof CaptureOptions, "viewport" | "type">> {
  type: CaptureType;
  viewport?: ViewportInput | ViewportInput[];
}

interface ExecutionOptions extends Pick<CaptureOptions, Exclude<keyof CaptureOptions, "viewport" | "type">> {
  type: CaptureType;
  viewport?: DeviceOrViewport | DeviceOrViewport[];
}

/**
 * The capture result object containing informations
 * about the capture output and the execution process
 */
interface Result {
  input: string;
  output: any;
...
/**
 * Capturer constructor options
 * used to customize browser instance
 * and set the default request properties
 * such as headers, default viewport and more
 */
...
/**
 * The capturer class that will run with customised settings
 * such as custom output path, default headers, default viewport and more
 */
export class WebpageCapture extends EventEmitter {
   ...
   */
  public async base64(source: string, opts: Partial<CaptureOptions> = {}) {
   ...
   * and it return a single element instead of an array of results
   */
  public async buffer(source: string, opts: Partial<CaptureOptions> = {}) {
    const [res] = await this.capture(source, _.merge(opts, {
      type: "buffer",
    }));
    return res;
   ...
   * and it return a single element instead of an array of results
   */
  public async file(source: string, output: string, opts: Partial<CaptureOptions> = {}) {
   ...
   */
  public async capture(sources: string | string[], opts: CaptureOptions = {}): Promise<Result[]> {
    // Ensure default options
    const options: RunOptions = _.merge({
      type: "png",
      options: {},
    }, opts);
   ...
   */
  private async preparePage(source: string, options: ExecutionOptions) {
    const {
      viewport,
      scripts,
      styles,
   ...
   */
  private async render(source: string, options: ExecutionOptions) {
   ...
   */
  private async run(source: string, options: RunOptions) {
    ...
    if (options.viewportCategory) {
      ...
      if (_.isArray(options.viewport)) {
        viewports = options.viewport.map((v) => {
          const viewport = getViewport(v);
          if (_.isUndefined(viewport)) {
            throw new Error(`Invalid viewport "${v}"`);
          }
          return viewport;
        });
      } else if (_.isString(options.viewport)) {
