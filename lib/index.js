...
const webpageCapture = (input, options = {}, callback) => {
  ...
  options.output = options.output || `./output/${hostname}.${options.renderOptions.format}`;
  ...
  phantom.create(phantomOptions).then(ph => {
    ...
    _p.on('onResourceError', function (res) {
      const rsUrl = Url.parse(res.url);
      if (options.onlySuccess && (rsUrl.hostname && rsUrl.hostname.indexOf(hostname) > -1)) {
        debug('An error occurred with resouce at url:', url);
        _p.stop();
      }
    });
