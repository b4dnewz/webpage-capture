...
const renderAsHTML = (page, content, options) => {
  ...
  return new Promise((resolve, reject) => {
    let func = function (status) {
      ...
      if (status !== 'success') {
        return reject(
          new Error(`The response has not a success status: ${status}`)
        );
      }
...
const webpageCapture = (input, options = {}, callback) => {
  ...
  // Prepare viewports
  options.viewport = common.getViewports(options.viewport);

  // Prepare global user agent option
  ...
  // Validate the render format
  options.renderOptions.format = common.validateFormat(
