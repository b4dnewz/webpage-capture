...
class WebpageCapture extends EventEmitter {
  constructor(opts = {}) {
    ...
    try {
      ...
      throw new Error('The output directory', this.options.outputDir, 'does not exist.');
    }
  }
  ...
  async capture(sources, opts = {}) {
    ...
    for (let i = 0; i < sources.length; i++) {
      ...
      const outPath = this.getOutPath(opts.outputName ? {name: opts.outputName} : source, opts.type);
      const result = {
        url: source,
        path: outPath
      };
...
WebpageCapture.prototype.run = async function (source, options) {
  if (!options.viewport) {
    await this.render(source, options);
    return;
  }
 ...
 */
WebpageCapture.prototype.preparePage = async function (url, options) {
 ...
 */
WebpageCapture.prototype.getOutPath = function (input, type) {
  const parts = [];
  switch (typeof input) {
    case 'string': {
