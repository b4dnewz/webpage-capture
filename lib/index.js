...
const openFromString = (page, string, options) => {
  ...
  if (urlObject.hostname) {
    ...
    let opts = Object.assign({}, options, {
      output: `${options.output}/${urlObject.hostname}.${options.renderOptions.format}`
    });
    promise = renderAsURL(page, url.format(urlObject), opts);
  } else {
    debug('Rendering input string as HTML body.');
    // Ensure output property
    let opts = Object.assign({}, options, {
      output: `${options.output}/${Date.now()}.${options.renderOptions.format}`
    });
    promise = renderAsHTML(page, string, opts);
  }
  ...
  return new Promise((resolve, reject) => {
    async.mapSeries(array, (entry, cb) => {
      ...
      if (typeof entry === 'object') {
        ...
        // Open it as string
        openFromString(page, value, options)
          .then(res => {
...
const webpageCapture = (input, options = {}, callback) => {
  ...
  phantom.create(phantomOptions, {
    ...
    debug('Start to parse the input.');
    return new Promise((resolve, reject) => {
